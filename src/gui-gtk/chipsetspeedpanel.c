...
static void on_leds_on_screen_changed (GtkWidget *w, ChipsetSpeedPanel *panel);
...
static void chipsetspeedpanel_init (ChipsetSpeedPanel *panel)
{
	...
	panel->leds_on_screen_widget = gtk_check_button_new_with_label ("Status Line"), 1, 2, GTK_EXPAND, GTKUTIL_ROW_END,
	GTKUTIL_TABLE_END
    );
    ...
    gtk_signal_connect (GTK_OBJECT (panel->leds_on_screen_widget), "toggled", GTK_SIGNAL_FUNC (on_leds_on_screen_changed), panel);
}
...
static void on_leds_on_screen_changed (GtkWidget *w, ChipsetSpeedPanel *panel)
{
     panel->leds_on_screen = gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (panel->leds_on_screen_widget));
     gtk_signal_emit_by_name (GTK_OBJECT(panel), "leds-on-screen-changed");
}
...
{
    gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (panel->leds_on_screen_widget), leds_on_screen != 0);
}
